...
use axum::{Router, extract::DefaultBodyLimit, routing::{delete, get, post, patch, put}};
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/servers/{server_id}/members/{user_id}", delete(servers::kick_member))
        .route("/servers/{server_id}/bans/{user_id}", post(servers::ban_member))
        .route("/servers/{server_id}/bans/{user_id}", delete(servers::unban_member))
        .route("/servers/{server_id}/search", get(messages::search_messages))
        // Scheduled events
        .route("/servers/{server_id}/events", get(events::list_events))
        .route("/servers/{server_id}/events", post(events::create_event))
        .route("/servers/{server_id}/events/{event_id}/rsvp", post(events::rsvp_event))
        .route("/servers/{server_id}/events/{event_id}", delete(events::delete_event))
        // Roles & permissions
        .route("/servers/{server_id}/me/permissions", get(roles::my_permissions))
        .route("/servers/{server_id}/roles", get(roles::list_roles))
        .route("/servers/{server_id}/roles", post(roles::create_role))
        .route("/servers/{server_id}/roles/{role_id}", delete(roles::delete_role))
        .route("/servers/{server_id}/members/{user_id}/roles", get(roles::member_role_ids))
        .route("/servers/{server_id}/members/{user_id}/roles/{role_id}", put(roles::assign_role))
        .route("/servers/{server_id}/members/{user_id}/roles/{role_id}", delete(roles::unassign_role))
        // Invites
        ...
        // Channels
        .route("/servers/{server_id}/channels", get(channels::list_channels))
        .route("/servers/{server_id}/channels", post(channels::create_channel))
        .route("/servers/{server_id}/categories", post(channels::create_category))
        .route("/servers/{server_id}/categories/{category_id}", delete(channels::delete_category))
        .route("/servers/{server_id}/channels/{channel_id}/category", put(channels::set_channel_category))
        ...
        .route("/channels/{channel_id}/messages/{id}", patch(messages::edit_message))
        .route("/channels/{channel_id}/messages/{id}", delete(messages::delete_message))
        .route("/channels/{channel_id}/polls", post(polls::create_poll))
        .route("/channels/{channel_id}/polls/{message_id}/vote", post(polls::vote_poll))
        .route("/channels/{channel_id}/pins", get(messages::list_pins))
        .route("/channels/{channel_id}/messages/{id}/pin", post(messages::pin_message))
        .route("/channels/{channel_id}/messages/{id}/pin", delete(messages::unpin_message))
        .route("/channels/{channel_id}/messages/{message_id}/save", post(saved::save_message))
        .route("/channels/{channel_id}/messages/{message_id}/save", delete(saved::unsave_message))
        // Reactions — toggle with POST (idempotent add/remove)
        .route("/channels/{channel_id}/messages/{message_id}/react/{emoji}", post(reactions::toggle_reaction))
        // Files — large streaming uploads, capped at 2 GiB to bound disk-exhaustion DoS
        .route("/upload", post(files::upload_file).layer(DefaultBodyLimit::max(2 * 1024 * 1024 * 1024)))
        ...
        .route("/servers/{server_id}/emojis/{emoji_id}", delete(delete_emoji))
        // Avatar upload
