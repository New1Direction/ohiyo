...
| 4 | **Every message has a clear state** | ✅ | **Fixed.** Optimistic message renders **dimmed (pending)** → **sent** on echo, or **failed** with an inline **Retry / Delete** affordance if the send errors (no silent vanish). e2e-locked (`14-failed-retry`). Read receipts now ship (e2e-locked, `17-receipts`). |
...
| 8 | **Presence is smoothed** | 🟡 | Typing is debounced server-side (2.5 s cooldown) + client TTL (5 s) — not flickery. Online/offline via gateway. Read receipts now ship (e2e-locked, `17-receipts`). |
...
3. **Hardening nice-to-haves.** ThumbHash blur-up (gate 5), server idempotency key so outbox flush can never duplicate (gate 7), per-channel attachments/reply (gate 6).
