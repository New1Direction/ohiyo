# Kikkacord TURN (coturn)

Production NAT traversal. STUN alone fails for ~10–20% of users behind symmetric
NATs; TURN relays their media so calls connect anywhere. The Kikkacord server
...
1. **Generate a secret:** `openssl rand -hex 32`. Put the value in this directory's
   `.env` as `TURN_SECRET=…` **and** in the Kikkacord server's environment as
...
5. **Point the app at it:** on the Kikkacord server set
...
```
TURN_SECRET=<same value the Kikkacord server uses>
